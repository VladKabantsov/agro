    ...
    <h2>Добавление товара</h2>    
        <form action="/goods" method="POST">
          ...
          <div class="form-group{{ $errors->has('barcode') ? ' has-error' : '' }}">
            ...
            <div class="form-group{{ $errors->has('сategory') ? ' has-error' : '' }}">
              <label>Категория</label>
              <select class="selectpicker form-control" id="сategory" name="сategory" data-live-search="true">
              <?php foreach ($сategories as $k => $row): ?>
                  <option value="{{ $k }}">{{ $row->cat_name }}</option> 
              <?php endforeach; ?>      
              </select>
              @if($errors->has('сategory'))
                <span class="help-block">{{ $errors->first('сategory') }}</span>
              @endif
            </div>
